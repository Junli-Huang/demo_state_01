...
if (!pg.root) {
  ...
  canvas.addEventListener('pointerdown', event => {
    ...
    if (draggedEntity) { canvas.setPointerCapture(event.pointerId); event.preventDefault(); }
  });
...
cards.innerHTML = entities.map(entity => { const v = entity.getVariables(), name = entityName(entity), attrs = v.get('Attributes').toJSObject() || {}, sources = ensureContributions(entity).filter(item => item.attribute === 'Fire'), sourceHtml = sources.length ? sources.map(item => `<div class="sp-row"><span>↳ ${item.source}</span><span>+${item.amount}${item.remaining == null ? '' : ` · ${Math.max(0, item.remaining).toFixed(1)}s`}</span></div>`).join('') : '<div class="sp-row"><span>↳ No Sources</span><span>—</span></div>', s = (v.get('States').toJSObject() || {}).Burning || {}, active = !!s.Active, phase = active ? 'Exit' : 'Enter', progress = active ? (s.ExitProgress || 0) : (s.EnterProgress || 0), duration = active ? (s.ActiveDuration || 0) : (s.InactiveDuration || 0); if (entity.setColor) entity.setColor(active ? '255;105;66' : '255;255;255'); return `<div class="sp-card ${active ? 'active' : ''}"><div class="sp-row"><span class="sp-name">${name}</span><b>${active ? '🔥 BURNING' : 'INACTIVE'}</b></div><div class="sp-row"><span>ATTR · Fire</span><b>+${attrs.Fire || 0}</b></div>${sourceHtml}<div class="sp-row"><span>${phase} Progress</span><span>${Math.round(progress)}/100</span></div><div class="sp-meter"><i style="width:${Math.min(100, progress)}%"></i></div><div class="sp-row"><span>${active ? 'Active' : 'Inactive'} Duration</span><span>${duration.toFixed(1)}s</span></div>${active && !s.Granted ? `<div class="sp-row"><span>Fire grant</span><span>${Math.min(5, duration).toFixed(1)}/5.0s</span></div>` : ''}${!active && s.Granted ? `<div class="sp-row"><span>Fire removal</span><span>${Math.min(10, duration).toFixed(1)}/10.0s</span></div>` : ''}</div>`; }).join('');
